...
#validation
 ...
 def checkid(self, contact):
     ...
     if len(str(contact)) == 0:
         ...
         messagebox.showerror("Invalid", "Only numbers allowed in Attendabce id")
         return False
...
#===============EXPORT CSV
 ...
 def update(self):
     ...
     elif not any(ch.isdigit() for ch in time) or len(str(time))!=0:
         msg = 'Please Enter valid Time'
         messagebox.showerror(
             "Error", msg, parent=self.root)
     elif not any(ch.isdigit() for ch in date) or len(str(date))!=0:
         msg = 'Please Enter valid Time'
         messagebox.showerror(
             "Error", msg, parent=self.root)
